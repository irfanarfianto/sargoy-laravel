<div class="drawer-side overflow-y-auto">
    ...
    <ul class="menu p-4 w-72 min-h-full bg-blue-950 text-base-100 mt-16 fixed">
        <!-- Dashboard Section -->
        ...
        <li>
            <a href="{{ route('dashboard.product.index') }}"
                ...
                List Produk
                @if (isset($unverifiedProductCount) && auth()->user()->hasRole('admin'))
                    <span class="badge badge-secondary">{{ $unverifiedProductCount }}</span>
                @endif
            </a>
        </li>
        @if (auth()->user()->hasRole('admin'))
        ...
        <li>
            <a href="{{ route('reviews.index') }}"
                ...
                Reviews
                @if ($newReviewCount > 0)
                    <span class="badge badge-secondary">+{{ number_format($newReviewCount, 0, ',', '.') }}</span>
                @endif
            </a>
        </li>
        ...
        <li>
            <button onclick="document.getElementById('keluar').showModal()"
                class="text-white hover:bg-red-700 px-4 py-2 rounded">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="size-6">
                    <path stroke-linecap="round" stroke-linejoin="round"
