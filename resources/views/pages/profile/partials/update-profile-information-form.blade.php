...
<form id="profile-update-form" method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
    ...
    <div class="space-y-2">
        <div>
            ...
            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $user->name)"
                required autofocus autocomplete="name" maxlength="80" />
            <span id="name-char-count" class="text-xs text-gray-600">80</span>
    ...
    <!-- WhatsApp Number, Address, Birthdate, Gender, Bio Fields -->
    <div class="space-y-2">
        <x-input-label for="no_wa" :value="__('WhatsApp Number')" />
        <x-text-input id="no_wa" name="visitor[no_wa]" type="text" class="mt-1 block w-full" :value="old('visitor.no_wa', $profile->no_wa ?? '')" />
        ...
        <x-select id="gender" name="visitor[gender]" class="mt-1 block w-full">
            <option value="male" @if (old('visitor.gender', $profile->gender ?? '') === 'male') selected @endif>
                {{ __('Male') }}</option>
            <option value="female" @if (old('visitor.gender', $profile->gender ?? '') === 'female') selected @endif>
                {{ __('Female') }}</option>
            <option value="other" @if (old('visitor.gender', $profile->gender ?? '') === 'other') selected @endif>
                {{ __('Other') }}</option>
        </x-select>
